...
// 	}
// }
// function money_add($itemid = 0, $userid, $amount, $editor, $status, $reason = '', $note = ''){
// 	global $db, $DT_TIME, $DT_PRE;
// 	if($userid && $amount && $itemid){
// 		$db->query("UPDATE {$DT_PRE}finance_record SET status=3, editor='$editor', reason='$reason', note='$note', updatetime='$DT_TIME' WHERE itemid='$itemid'");
// 	}
// 	if($userid && $amount) $db->query("UPDATE {$DT_PRE}member SET money=money+{$amount} WHERE userid='$userid'");
// }
function Money_Add_Record($userid, $username, $typeid, $gameid, $money, $rebate, $editor, $reason = '', $note = ''){
	global $db, $DT_TIME, $DT_PRE, $DT, $USER;
	// 字段 userid, username, 编号(no), 类型(typeid), 游戏(gameid), 充值金额(money), 实际金额(balance), 手续费(fee), rebate(返利), 总金额(amount), addtime, reason(原因), note(备注), status(状态), editor(审核人);
	// status 1为已提交OR审核中/2为审核失败/3为审核通过
	if( $userid && $username ){
		$MOD = cache_read("module-2.php");
		$post = array();
		$fileds = array('userid', 'username', 'no', 'typeid', 'gameid', 'money', 'balance', 'fee', 'rebate', 'amount', 'addtime', 'reason', 'note', 'status', 'editor');
		$Ocode = array('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J');
		$no = $Ocode[intval(date('Y'))-(timetodate($DT_TIME, 1)-1)].'-'.strtoupper(dechex(date('m'))).date('d').substr(time(),-5);
		$post['userid'] = $userid;
		$post['username'] = $username;
		$post['no'] = $no;
		$post['typeid'] = intval($typeid) > 0 ? intval($typeid) : 0;
		$post['gameid'] = intval($gameid) > 0 ? intval($gameid) : 0;
		$post['money'] = floatval($money);
		if( $MOD['isFeeAmount'] > 0 ){
			$post['balance'] = $post['money']-$MOD['isFeeAmount'];
		}else{
			$post['balance'] = $post['money'];
		}
		$post['fee'] = $MOD['isFeeAmount'];
		$post['rebate'] = $rebate;
		$post['amount'] = $post['money'];
		...
		$sqlk = $sqlv = '';
		foreach($post as $k=>$v) {
			if(in_array($k, $fileds)) {$sqlk .= ','.$k; $sqlv .= ",'$v'";}
		}
		$sqlk = substr($sqlk, 1);
		$sqlv = substr($sqlv, 1);
		$db->query("INSERT INTO {$DT_PRE}finance_record ($sqlk) VALUES ($sqlv)");
		$post['dtype'] = explode('|', $MOD['history']);
		$post['_status'] = array('<span style="color:blue;">未知状态</span>', '<span style="color:blue;">等待审核</span>', '<span style="color:red;">审核失败</span>', '<span style="color:green;">审核成功</span>', '<span style="color:green;">人工审核</span>');
		$post['title'] = $DT['sitename'].' - 存款提醒';
		$content = ob_template('deposit_tips', 'mail', $post);
		if($USER['email'] && is_email($USER['email']) && $DT['access_tips'] > 0) send_mail($USER['email'], $post['title'], stripslashes($content));
	}
}
